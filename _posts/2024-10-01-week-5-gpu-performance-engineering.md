...
CUDA (Compute Unified Device Architecture) is NVIDIA's parallel computing platform and programming model that allows developers to use GPUs for general-purpose computing, not just graphics. Think of it as the bridge that transforms a graphics card designed for rendering pixels into a massively parallel processor that can accelerate everything from machine learning to scientific simulations. CUDA provides both the low-level hardware interface and the high-level programming abstractions that make GPU programming accessible to developers.<span class="margin-note">**CUDA's Revolutionary Origins & Academic Foundations**: CUDA's development traces directly to Pat Hanrahan's graphics research group at Stanford, where the Brook project pioneered stream programming for GPUs in the early 2000s. Brook introduced the radical idea that GPUs could be programmed as parallel processors rather than just graphics renderers. Ian Buck, who worked on Brook during his PhD at Stanford, joined NVIDIA in 2004 and led CUDA's development, bringing these academic insights into a commercial platform. Before CUDA, developers had to express general-purpose computations as graphics operations, essentially tricking the GPU into doing math by pretending it was rendering pixels. CUDA was introduced in 2006 as a radical departure from this approach. The "Compute Unified Device Architecture" name was deliberate: it unified graphics and compute workloads under a single programming model, transforming GPUs from specialized graphics processors into general-purpose parallel computers. The academic foundations run even deeper: Bill Dally's work on streaming multiprocessors at Stanford provided the architectural principles that became the backbone of modern GPU design. When he joined NVIDIA in 2009, he brought these insights directly into the company's architectural roadmap. And given this is a Harvard blog, it would only be fair to mention that at Harvard, H.T. Kung's 1982 work on systolic arrays provided the mathematical foundation for Google's TPU architecture. When people ask "what do academics actually do?" This is what we do: create the foundational insights that enable trillion-dollar industries. This Stanford-to-NVIDIA and Harvard-to-Google pipeline exemplifies how academic research in systems can create entire industries.</span> But CUDA isn't just a programming language. It's an entire ecosystem that learned from Cell's mistakes by providing manageable abstractions while still exposing the performance potential of parallel hardware.
...
Professor Rush's perspective from [Cursor](https://cursor.sh/), a company building AI-powered development tools used by millions of developers, illuminated the gap between research demonstrations and production deployment. (Note: The views expressed are Professor Rush's own and do not necessarily reflect Cursor's official positions.) Several key insights emerged:
...
*For detailed readings, slides, and materials for this week, see [Week 5 in the course schedule](/cs249r_fall2025/course/schedule/#week-5-gpu-performance-engineering).*
